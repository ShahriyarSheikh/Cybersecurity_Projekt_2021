...
@Component({
    selector: 'app-selloffer',
    templateUrl: './selloffer.component.html',
    styleUrls: ['./selloffer.component.scss'],
    providers: [CreateNewOffer]
})
export class SellOfferComponent implements OnInit {
        ...
        if (this.selectedFiatAccountIndex != undefined && this.form.value.fiatQuantity != undefined && this.form.value.ethQuantity != undefined
            ...
            if (this.contract != undefined) {
                await promisify(cb => this.contract.placeSellOffer(
                    ...
                    this.web3.toWei(0.1), cb)).catch(error => Loading.emit(false));
                this.clearForm();
            }
        }
    }
