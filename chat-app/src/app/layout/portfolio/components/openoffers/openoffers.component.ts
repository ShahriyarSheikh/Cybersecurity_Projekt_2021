...
@Component({
    selector: 'app-openoffers',
    templateUrl: './openoffers.component.html',
    styleUrls: ['./openoffers.component.scss']
})
export class OpenOffersComponent implements OnInit {
    ...
    async LoadSellTradeDetailsFromTradeHash(hash) {
        ...
        if (offerDetails[5] == EmptyAddress && offerDetails[4] == this.coinbase) {
    ...
    async LoadBuyTradeDetailsFromTradeHash(hash) {
        ...
        if (offerDetails[4] == EmptyAddress && offerDetails[5] == this.coinbase) {
